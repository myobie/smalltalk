...
export class TaskNotFoundError extends CoordError {
  readonly identity: string;
  readonly filename: string;
  constructor(identity: string, filename: string) {
    super(
      'TASK_NOT_FOUND',
      `task not found: ${identity}/tasks/${filename}`,
      { identity, filename }
    );
    this.identity = identity;
    this.filename = filename;
...
export class TasksSingleWriterError extends CoordError {
  readonly identity: string;
  readonly filename: string;
  readonly ownerIdentity: string;
  constructor(identity: string, filename: string, ownerIdentity: string) {
    super(
      'TASKS_SINGLE_WRITER',
      `tasks are single-writer: ${filename} belongs to ${ownerIdentity}, not ${identity}. Each identity edits only its own tasks/ folder; others read via sync.`,
      { identity, filename, ownerIdentity }
    );
    this.identity = identity;
    this.filename = filename;
...
export class InvalidTaskTitleError extends CoordError {
  readonly value: string;
  constructor(value: string) {
    super(
      'INVALID_TASK_TITLE',
      `task title must contain at least one alphanumeric character: ${JSON.stringify(value)}`,
      { value }
    );
    this.value = value;
  }
...
export class InvalidTaskStateError extends CoordError {
  readonly value: string;
  constructor(value: string) {
    super(
      'INVALID_TASK_STATE',
      `invalid task state: ${value} (expected one of: todo, doing, done, blocked)`,
      { value }
    );
    this.value = value;
  }
